...
if let Err(_) = perf_reader.read(&sample) {
    errorf!("perfbuffer must be paused before reading");
    return 1;
}
```
The above code prints an error before exiting:

```bash
ERRO[0002] perfbuffer must be paused before reading
```
...
```bash
sudo ig image build . -t <custom_image_tag> --builder-image=<gadget_builder_docker_image>:<tag>
```
...
```bash
sudo ig run <custom_image_tag> --verify-image=false
```
...
To support Rust WASM, the `gadget-builder` image size increased from **2.95 GB** to **4.18 GB**. A future improvement is to check how to only include the necessary toolchain (`wasm32-wasip1`, `cargo`, `rustc`) and exclude others like `clippy`, `rustfmt` to save space.

## The Experience

I learned that open source is not about free software — it’s about collaboration and robustness. Under the mentorship of amazing maintainers, I submitted code that received detailed review (sometimes [150](https://github.com/inspektor-gadget/inspektor-gadget/pull/4275) comments!).
