...
If I were to give a minimal and easy to understand example for this then the following snippet in the cli operator would be appropriate: 

In order to test the following snippet of code that is responsible for displaying the data when the output mode of the datasource is yaml, I had to go through the extensive boilerplate code of the `PreStart()` function that cannot be tested without instantiating the operator itself.
...
In this way, for the first half of the mentorship, I designed and implemented extensive unit tests for these operators:
...
![Coverage](../static/media/2025-05-28-implementing-unit-tests.png)
...
The existing unit tests for the `trace_open` gadget were generating the open syscall explicitly and testing the gadget in a more end-to-end manner. I had the same questions here: why are we triggering the entire gadget if we just have to test some BPF logic written in C?
