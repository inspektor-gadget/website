...
Given that there are two types of runtimes — a local one (used by `ig`) and a gRPC-based one (used by `gadgetctl` and `kubectl-gadget`) — the implementation for the gRPC version was slightly more involved.
...
With the runtime now capable of handling `ExtraInfo`, the next step was to generate this data within the operators themselves. This was implemented in the `init` function of each operator (e.g., `WASM`, `eBPF`, `oci-handler`). Below is an example from the `oci-handler` operator:
...
Once the "extra" info is stored in the `gadgetctx`, the final task is to serialize it — and in the case of gRPC clients, potentially load it back into the client-side `gadgetctx`.
...
```bash
$ sudo ig image inspect audit_seccomp:main --extra-info=oci.manifest
{
  "annotations": {
...
```bash
$ sudo ig image inspect audit_seccomp:main --show-datasources
[
  {
    "name": "seccomp",
    "type": 1,
...
One of the coolest things we added to the `ig image inspect` command is support for **Mermaid graphs**. We wanted to give gadget developers an easy way to visualize how programs and maps interact under the hood—so we introduced two fields specifically for this: `ebpf.flowchart` and `ebpf.sequence`.
